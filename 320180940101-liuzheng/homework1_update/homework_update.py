...
class sl_hour_cnt:
    ...
    def get_list(self):
        ...
        try:
            ...
            for sl in range(1, self.rev_range + 1):
                ...
                self.commit_cnt_list.append(commit_cnt)
                sublevels.append(sl)
            ...
            # # create a dataframe
            # dt = np.array([sl, release_hours, commit_cnt_list])
            # data = dt.transpose()
            # print(data)
            # tags = ['lv', 'hour', 'bugs']
    ...
    def data_frame(self):
        ...
        # create a dataframe
        dt = np.array([self.lv2,self.release_hours, self.commit_cnt_list])
        data = dt.transpose()

        n=len(self.lv2)+1
        tags = ['lv', 'hour', 'bugs']
        df = pd.DataFrame(data=data, columns=tags, index=list(range(1,n)))
        print(df)
    ...
    def lost_report(self):
        ...
        # print(LIST2)
        outlist = []
        for column, row in zip(LIST1, LIST2):
            outlist.append('The null value in ' + str(row) + 'th line is ' + str(column) + '\n')

        # print(outlist)
        f = open('Null_report.txt', 'w')
